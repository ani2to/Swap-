...
def send_loading_animation(chat_id):
    ...
    for i, text in enumerate(loading_steps):
        try:
            ...
            time.sleep(1.5)  # Shorter delay for faster experience
        except:
            pass
    return message_id
    ...
    try:
        ...
        if chat_id not in user_data:
            ...
            if user_data[chat_id]["state"] == WAITING_FOR_TARGET:
                ...
                processing_msg = bot.reply_to(message, "🔄 Starting face swap process...")
                
                loading_msg_id = send_loading_animation(chat_id)
                
                source_b64 = base64.b64encode(user_data[chat_id]["source"]).decode('utf-8')
                target_b64 = base64.b64encode(user_data[chat_id]["target"]).decode('utf-8')
        ...
        bot.reply_to(message, "⏰ Request timeout! Please try again with smaller images.")
        if chat_id in user_data:
            del user_data[chat_id]
    except Exception as e:
...
def handle_text(message):
    try:
        ...
        if chat_id in user_data:
            ...
            bot.reply_to(message, "👋 Welcome! Use /swap to begin face swapping! ✨")
    except Exception as e:
